...
async def test_google_oauth_new_user(monkeypatch, mock_db):
    ...
    response = client.post(
        "/auth/oauth/google",
        json={"credential": "valid-credential"}
    )
...
async def test_forgot_password_success(monkeypatch, mock_db, mock_redis):
    ...
    assert any(k.startswith("pwd_reset:") for k in mock_redis.store.keys())
...
async def test_reset_password_success(monkeypatch, mock_db, mock_redis):
    ...
    mock_redis.store[f"pwd_reset:{token}"] = "user@example.com"
    ...
    mock_db.commit.assert_awaited_once()
    assert f"pwd_reset:{token}" not in mock_redis.store
...
async def test_reset_password_expired_token(monkeypatch, mock_db, mock_redis):
    ...
    response = client.post(
        "/auth/reset-password",
        json={"token": "expired-token", "email": "user@example.com", "new_password": "NewSecurePassword123!"}
    )
...
async def test_refresh_token_rotation_success(monkeypatch, mock_db, mock_redis):
    ...
    # Mock user query
    user = User(id=uuid.UUID(user_id), email="user@example.com", role="user", is_active=True)
    mock_execute_result = MagicMock()
    mock_execute_result.scalar_one_or_none.return_value = user
    mock_db.execute.return_value = mock_execute_result
