...
async def create_review(db: AsyncSession, review_in: ReviewCreate, user_id: uuid.UUID, user_name: str) -> Review:
    new_review = Review(
...
async def update_review(db: AsyncSession, review_id: uuid.UUID, user_id: uuid.UUID, rating: int, text: str) -> Review | None:
...
async def delete_review(db: AsyncSession, review_id: uuid.UUID, user_id: uuid.UUID) -> bool:
