...
async def create_cart(
    db: AsyncSession, user_id: uuid.UUID, cart_in: CartCreate
) -> Cart:
...
async def add_item(
    db: AsyncSession, user_id: uuid.UUID, cart_id: uuid.UUID, item_in: CartItemCreate
) -> Cart | None:
    cart = await get_cart(db, user_id, cart_id)
    if not cart:
        return None
...
async def remove_item(
    db: AsyncSession, user_id: uuid.UUID, cart_id: uuid.UUID, item_id: uuid.UUID
) -> bool:
    cart = await get_cart(db, user_id, cart_id)
    if not cart:
        return False
