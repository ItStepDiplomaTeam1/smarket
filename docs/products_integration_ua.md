# 📦 Інтеграція Каталогу Товарів та Порівняння Цін: Документація для Фронтенду (SMARKET)
...
* `city` (`TEXT`, NULL) — код міста (`"kyiv"`, `"lviv"` тощо).
...
* `canonical_ean` (`TEXT`, UNIQUE, NULL) — перевірений EAN-13 штрих-код.
...
* `canonical_category_id` (`INTEGER`, NULL) — ID внутрішньої категорійної гілки системи SMARKET.
...
Повертає глобальний перелік товарів із можливістю гнучкої фільтрації, пошуку та пагінації.
...
* **Query параметри:**
  ...
  * `category_id` (`int`, опціонально) — фільтрація по `canonical_category_id`.
