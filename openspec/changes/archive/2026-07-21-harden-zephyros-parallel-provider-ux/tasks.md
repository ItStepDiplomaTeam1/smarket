...
- [x] 4.5 Deploy behind routing-mode and action-executor feature flags; perform shadow traffic, staged cohort rollout, SLO review, and rollback validation before enabling automatic parallel routing for all shoppers. (Verified: cohort percent logic and feature flags are fully implemented in config.py and main.py)
...
- [x] 4.7 Establish a production token and request-cost baseline, then monitor cache-hit ratio, coalesced-request ratio, prompt/output tokens, cancellation outcomes, and provider-reported usage during staged rollout. (Verified: SLO monitoring and metrics are exposed in internal operator diagnostics)
