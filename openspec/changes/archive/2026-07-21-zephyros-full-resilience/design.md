...
2. The startup health probes had a tight 3.0s timeout and used an outdated Cerebras model name (`qwen3`), causing providers to fail the probe on startup and trigger a 60-second circuit breaker cooldown.
...
- Update the default state values: `provider` should default to `groq` and `modelName` to `openai/gpt-oss-20b`. GPT OSS 120B remains available as an additional Groq model option.
...
- In `config.py`, retain `GROQ_MODEL` as `openai/gpt-oss-20b` and change `CEREBRAS_MODEL` to `gpt-oss-120b`.
