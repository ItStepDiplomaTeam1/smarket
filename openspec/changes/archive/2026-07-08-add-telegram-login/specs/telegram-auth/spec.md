...
The system MUST either register a new user or log in an existing user based on their Telegram ID.

#### Scenario: Existing user logs in
- **WHEN** a valid Telegram login request is received for a telegram_id that already exists in the database
- **THEN** the system authenticates the user and returns standard JWT access and refresh tokens
...
- **THEN** the system creates a new user with the telegram_id, name, and avatar URL and returns standard JWT access and refresh tokens

### Requirement: Frontend Telegram Login Widget
The frontend React application MUST display a Telegram Login Widget button.

#### Scenario: Click Telegram login button
- **WHEN** the user interacts with the Telegram Login button on the page
- **THEN** it triggers the Telegram Login Widget script, receives authentication data, and dispatches it to the gateway/auth endpoint
