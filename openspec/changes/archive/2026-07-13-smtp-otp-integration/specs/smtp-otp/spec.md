...
#### Scenario: Sending email with complete configuration
- **WHEN** SMTP credentials are fully configured and a request to send an email is made
- **THEN** the system connects asynchronously, establishes a secure connection, transmits the email, and logs a success status
...
#### Scenario: Send verification OTP email
- **WHEN** requested to send an OTP email to a user
- **THEN** the system reads the template, replaces `{{ code }}` with the OTP, and calls the asynchronous email dispatcher to send it
