    ...
    <section className="w-full font-inter flex-1 min-w-0 pb-[40px]">
      <div className="w-full">
        ...
        <div className="flex flex-wrap items-center gap-[4px] font-inter text-[13px] text-[#6D8279] mb-[12px]">
          ...
          <img src={rightVector} alt=">" className="w-[10px] h-[10px] object-contain mx-[2px] opacity-65 brightness-50 dark:brightness-100" />
          ...
          <img src={rightVector} alt=">" className="w-[10px] h-[10px] object-contain mx-[2px] opacity-65 brightness-50 dark:brightness-100" />
          <span className="text-[#94A3B8] font-semibold">Відгуки</span>
        </div>
        ...
        {/* Вкладки (Тільки Всі відгуки, без Чернетки) */}
        <div className="border-b border-[#E5E7EB] dark:border-[#265447]/30 mb-[24px]">
          <div className="flex gap-[24px]">
            <div className="pb-[8px] border-b-2 border-[#173B33] dark:border-[#4ADE80] cursor-pointer">
              <span className="font-inter text-[14px] font-semibold text-[#173B33] dark:text-white">Всі відгуки</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-[16px] mb-[24px]">
          {/* Сортування */}
          <div 
            className="h-[30px] border border-[#E5E7EB] dark:border-[#265447]/30 rounded-[6px] px-[12px] flex items-center justify-between cursor-pointer bg-white dark:bg-[#1C2723] gap-[8px]"
            onClick={() => setSortOrder(sortOrder === 'newest' ? 'oldest' : 'newest')}
          >
            <span className="font-inter text-[10px] font-normal text-[#173B33] dark:text-[#94A3B8] whitespace-nowrap">
              {sortOrder === 'newest' ? 'Сортування: нові спочатку' : 'Сортування: старі спочатку'}
            </span>
            <img src={rightVector} alt="v" className="w-[10px] h-[10px] object-contain rotate-90 opacity-65 brightness-50 dark:brightness-100" />
          </div>
        </div>
        ...
        <div className="flex flex-col">
          ...
          {/* Помилка */}
          {isError && (
            <div className="flex items-center gap-[8px] bg-[#2D1515] border border-[#EF4444]/30 rounded-[12px] px-[20px] py-[16px]">
          ...
          {!isLoading && !isError && (
            <>
              <div className="flex flex-col gap-[16px] min-h-[716px]">
              ...
              {/* Пагінація */}
              {totalReviews > 0 && (
                <div className="flex flex-col sm:flex-row items-center justify-between gap-[16px] sm:gap-0 mt-[24px]">
                  <span className="font-inter font-normal text-[14px] leading-[21.45px] text-[#6D8279] dark:text-[#94A3B8]">
                  ...
                  <div className="flex items-center gap-[13px]">
                    <button
                      ...
                      className={`w-[30px] h-[30px] bg-[#173B33] dark:bg-[#4ADE80] rounded-[6px] flex items-center justify-center transition-colors cursor-pointer border-none ${
                        currentPage === 1 ? 'opacity-30 cursor-default' : 'hover:bg-[#265447] dark:hover:bg-[#3DAE8B]'
                      }`}
                    >
                      <img src={leftVector} alt="Попередня" className="w-[6px] h-[10px] object-contain brightness-0 invert dark:invert-0" />
                    </button>
                    ...
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                      <button
                        ...
                        className={`w-[30px] h-[30px] rounded-[6px] flex items-center justify-center cursor-pointer transition-all ${
                          page === currentPage
                            ? 'bg-[#F6FAF8] border border-[#173B33] text-[#173B33] dark:bg-[#1C2723] dark:border-[#4ADE80] dark:text-[#4ADE80]'
                            : 'bg-white border border-[#E5E7EB] hover:bg-[#F6FAF8] text-[#6D8279] dark:bg-[#1C2723] dark:border-[#265447]/30 dark:hover:bg-[#173B33] dark:text-[#94A3B8]'
                        }`}
                      >
                        <span className={`font-manrope font-[800] text-[10px] ${
                          page === currentPage ? 'text-[#173B33] dark:text-[#4ADE80]' : 'text-[#6D8279] dark:text-[#94A3B8]'
                        }`}>
                          {page}
                        </span>
                      </button>
                    ...
                    <button
                      ...
                      className={`w-[30px] h-[30px] bg-[#173B33] dark:bg-[#4ADE80] rounded-[6px] flex items-center justify-center transition-colors cursor-pointer border-none ${
                        currentPage === totalPages ? 'opacity-30 cursor-default' : 'hover:bg-[#265447] dark:hover:bg-[#3DAE8B]'
                      }`}
                    >
                      <img src={rightVector} alt="Наступна" className="w-[6px] h-[10px] object-contain brightness-0 invert dark:invert-0" />
                    </button>
                  </div>
                </div>
              )}
