...
export const MyReceiptsModal: React.FC<MyReceiptsModalProps> = ({ isOpen, onClose }) => {
  ...
  const { data: receipts, isLoading, isError, error, refetch } = useGetMyReceipts(isOpen);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/50 backdrop-blur-sm">
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="my-receipts-title"
        className="bg-white dark:bg-[#111A17] rounded-t-2xl sm:rounded-2xl shadow-xl w-full sm:max-w-3xl overflow-hidden border border-gray-100 dark:border-[#265447]/30 transition-colors flex flex-col max-h-[90vh] sm:max-h-[80vh]"
      >
        
        {/* Header */}
        <div className="p-6 border-b border-gray-100 dark:border-[#265447]/20 flex justify-between items-center shrink-0">
          <h3 id="my-receipts-title" className="text-xl font-bold font-['Manrope'] text-[#173B33] dark:text-white flex items-center gap-2">
            <ShoppingBag className="w-5 h-5 text-[#265447] dark:text-[#3DAE8B]" /> Мої чеки
          </h3>
          <button
            onClick={onClose}
            aria-label="Закрити чеки"
